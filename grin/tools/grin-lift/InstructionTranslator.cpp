...
#include "grin/Support/RandomAccessIterator.h"
#include "grin/Support/Range.h"
...
public:
  ...
  std::string helperName() const {
    ...
    grin_assert(Helper != nullptr && Helper->name != nullptr);
    return std::string(Helper->name);
  }

  uint64_t pc() const {
    grin_assert(opcode() == PTC_INSTRUCTION_op_debug_insn_start);
...
static CmpInst::Predicate conditionToPredicate(PTCCondition Condition) {
  ...
  default:
    grin_unreachable("Unknown comparison operator");
  }
}
...
static Instruction::BinaryOps opcodeToBinaryOp(PTCOpcode Opcode) {
  ...
  default:
    grin_unreachable("PTC opcode is not a binary operator");
  }
}
...
static uint64_t getMaxValue(unsigned Bits) {
  if (Bits == 32)
    ...
    grin_unreachable("Not the number of bits in a integer type");
}
...
static unsigned getRegisterSize(unsigned Opcode) {
  ...
  case PTC_INSTRUCTION_op_set_label:
    return 0;
  default:
    grin_unreachable("Unexpected opcode");
  }
}
...
IT::newInstruction(PTCInstruction *Instr,
  ...
  if (!IsFirst) {
    ...
    if (CurrentIt == Builder.GetInsertBlock()->begin())
      grin_assert(JumpTargets.getBlockAt(PC) == Builder.GetInsertBlock());
    else
...
IT::TranslationResult IT::translateCall(PTCInstruction *Instr) {
  ...
  grin_assert(TheCall.OutArguments.size() <= 1);
...
IT::translate(PTCInstruction *Instr, uint64_t PC, uint64_t NextPC) {
  ...
  grin_assert(Result->size() == (size_t) TheInstruction.OutArguments.size());
...
IT::translateOpcode(PTCOpcode Opcode,
  ...
  if (RegisterSize == 32)
    ...
    grin_unreachable("Unexpected register size");
  ...
  case PTC_INSTRUCTION_op_qemu_st_i64: {
    ...
    grin_assert(MemoryAccess.access_type != PTC_MEMORY_ACCESS_UNKNOWN);
    ...
    case PTC_MO_64:
      ...
      break;
    default:
      grin_unreachable("Unexpected load size");
    }
    ...
    if (Opcode == PTC_INSTRUCTION_op_qemu_ld_i32
      ...
      grin_unreachable("Unknown load type");
    }
  }
  case PTC_INSTRUCTION_op_ld8u_i32:
  case PTC_INSTRUCTION_op_ld8s_i32:
  ...
  case PTC_INSTRUCTION_op_divu2_i64: {
    ...
    if (Opcode == PTC_INSTRUCTION_op_div2_i32
      ...
      grin_unreachable("Unknown operation type");
    }
  ...
  case PTC_INSTRUCTION_op_set_label: {
    ...
    if (ExistingBasicBlock == LabeledBasicBlocks.end()) {
      ...
      // Ensure it's empty
      grin_assert(Fallthrough->begin() == Fallthrough->end());
  ...
  case PTC_INSTRUCTION_op_brcond_i64: {
    ...
    if (Opcode == PTC_INSTRUCTION_op_br) {
      ...
      grin_unreachable("Unhandled opcode");
    }
  ...
  case PTC_INSTRUCTION_op_trunc_shr_i32:
    grin_unreachable("Instruction not implemented");
  default:
    grin_unreachable("Unknown opcode");
  }
}
