...
public:
  ...
  public:
    ...
    bool isSigned() const {
      grin_assert(Sign != UnknownSignedness && Sign != AnySignedness
                  && !Bottom);
    ...
    actualBoundaries(llvm::Type *Int64) const {
      grin_assert(!(Negated && isConstant()));
      ...
      grin_abort("The BV is unlimited");
    }
  ...
  private:
    uint64_t lowerExtreme() const {
      ...
      case InconsistentSignedness:
        return std::numeric_limits<uint64_t>::min();
      default:
        grin_unreachable("Unexpected signedness");
      }
    }
  ...
  public:
    ...
    public:
      ...
      bool operator==(BoundsIterator &Other) const {
        grin_assert(TheOSR.Factor == Other.TheOSR.Factor);
