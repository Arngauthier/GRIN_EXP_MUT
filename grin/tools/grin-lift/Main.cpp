...
opt<string>
  ExecutableArgs("exe-args", value_desc("arguments"), cat(MainCategory));
...
static void findFiles(const char *Architecture) {
  ...
  SearchPaths.push_back(std::string(INSTALL_PATH) + "/share/grin");
#endif
  ...
  grin_assert(LibtinycodeFound, "Couldn't find libtinycode and the helpers");
  grin_assert(EarlyLinkedFound, "Couldn't find early-linked.ll");
}
...
static int loadPTCLibrary(LibraryPointer &PTCLibrary) {
  ...
  if (ptc_load(LibraryHandle, &ptc, InputPath.c_str(), ExecutableArgs.c_str())
      != 0) {
