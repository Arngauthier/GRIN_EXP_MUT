...
BinaryFile::BinaryFile(std::string FilePath, uint64_t BaseAddress) :
  ...
  grin_assert(BinaryOrErr, "Couldn't open the input file");
  ...
  grin_assert(TheBinary->getFileFormatName().startswith("ELF"),
  ...
  if (TheArchitecture.pointerSize() == 32) {
    ...
    grin_assert("Unexpect address size");
  }
...
public:
  ...
  uint64_t addressAtOffset(uint64_t Offset) {
    grin_assert(HasAddress and HasSize);
    grin_assert(Offset <= Size);
  ...
  ArrayRef<T> extractAs(const std::vector<SegmentInfo> &Segments) const {
    ...
    grin_assert(Data.size() % TypeSize == 0);
  ...
  extractData(const std::vector<SegmentInfo> &Segments) const {
    ...
    for (const SegmentInfo &Segment : Segments) {
      if (Segment.contains(Address)) {
        ...
        if (HasSize) {
          grin_assert(AvailableSize >= Size);
          TheSize = Size;
        }
...
void BinaryFile::parseELF(object::ObjectFile *TheBinary, uint64_t BaseAddress) {
    ...
    for (auto &Section : *Sections) {
      ...
      if (NameOrErr) {
        ...
        if (Name == ".symtab") {
          ...
          grin_assert(SymtabShdr == nullptr, "Duplicate .symtab");
          SymtabShdr = &Section;
        } else if (Name == ".eh_frame") {
          grin_assert(not EHFrameAddress, "Duplicate .eh_frame");
          EHFrameAddress = relocate(static_cast<uint64_t>(Section.sh_addr));
          EHFrameSize = static_cast<uint64_t>(Section.sh_size);
          ehframeEndAddr = relocate(
            static_cast<uint64_t>(Section.sh_addr + Section.sh_size));
        } else if (Name == ".dynamic") {
          grin_assert(not DynamicAddress, "Duplicate .dynamic");
  ...
  for (Elf_Phdr &ProgramHeader : *ProgHeaders) {
    ...
    case ELF::PT_GNU_EH_FRAME:
      grin_assert(!EHFrameHdrAddress);
      EHFrameHdrAddress = relocate(ProgramHeader.p_vaddr);
      break;

    case ELF::PT_DYNAMIC:
      grin_assert(DynamicPhdr == nullptr, "Duplicate .dynamic program header");
      DynamicPhdr = &ProgramHeader;
      grin_assert(((not DynamicAddress)
  ...
  grin_assert((DynamicPhdr != nullptr) == (DynamicAddress.hasValue()));
  ...
  if (EHFrameHdrAddress) {
    ...
    if (EHFrameAddress) {
      grin_assert(*EHFrameAddress == Address);
    }
  ...
  if (DynamicPhdr != nullptr) {
    ...
    for (Elf_Dyn &DynamicTag : *DynamicEntries) {
      ...
      case ELF::DT_REL:
      case ELF::DT_RELA:
        grin_assert(TheTag == (HasAddend ? ELF::DT_RELA : ELF::DT_REL));
        ReldynPortion.setAddress(relocate(DynamicTag.getPtr()));
        break;

      case ELF::DT_RELSZ:
      case ELF::DT_RELASZ:
        grin_assert(TheTag == (HasAddend ? ELF::DT_RELASZ : ELF::DT_RELSZ));
    ...
    if (NeededLibraryNames.size() > 0)
      grin_assert(DynstrPortion.isAvailable());
...
uint64_t BinaryFile::symbolsCount(const FilePortion &Relocations) {
  ...
  grin_assert(Relocations.isExact());
...
BinaryFile::readRawValue(uint64_t Address, unsigned Size, Endianess E) const {
  ...
  for (auto &Segment : segments()) {
    ...
    if (Segment.contains(Address, Size) && Segment.IsReadable) {
      ...
      default:
        grin_abort("Unexpected read size");
      }
    }
  }
...
Label BinaryFile::parseRelocation(unsigned char RelocationType,
  ...
  case RD::Invalid:
  default:
    grin_abort("Invalid relocation type");
    break;
  }
}
...
void BinaryFile::registerRelocations(Elf_Rel_Array<T, HasAddend> Relocations,
  ...
  for (Elf_Rel Relocation : Relocations) {
    ...
    if (Dynsym.isAvailable() and Dynstr.isAvailable()) {
      ...
      grin_check(SymbolIndex < Symbols.size());
...
public:
  ...
  template<typename T>
  T readNext() {
    grin_assert(Cursor + sizeof(T) <= End);
  ...
  Pointer readPointer(unsigned Encoding, uint64_t Base = 0) {
    grin_assert((Encoding & ~(0x70 | 0x0F | dwarf::DW_EH_PE_indirect)) == 0);
    ...
    case dwarf::DW_EH_PE_sdata8:
      return readPointerInternal(readNext<int64_t>(), Encoding, Base);
    default:
      grin_unreachable("Unknown Encoding");
    }
  }

  void moveTo(uint64_t Offset) {
    const uint8_t *NewCursor = Start + Offset;
    grin_assert(NewCursor >= Cursor && NewCursor <= End);
    Cursor = NewCursor;
  }
...
private:
  ...
  Pointer readPointerInternal(T Value, unsigned Encoding, uint64_t Base) {
    ...
    if (Value != 0) {
      ...
      grin_assert(EncodingRelative == 0 || EncodingRelative == 0x10);
...
BinaryFile::ehFrameFromEhFrameHdr(uint64_t EHFrameHdrAddress) {
  ...
  grin_assert(R, ".eh_frame_hdr section not available in any segment");
...
void BinaryFile::parseEHFrame(uint64_t EHFrameAddress,
                              Optional<uint64_t> FDEsCount,
                              Optional<uint64_t> EHFrameSize) {
  grin_assert(FDEsCount || EHFrameSize);
  ...
  while (!EHFrameReader.eof()
    ...
    if (Length == 0) {
      grin_assert(EHFrameReader.offset() == EndOffset);
      continue;
    }
    ...
    if (ID == 0) {
      ...
      if (!AugmentationString.empty() && AugmentationString.front() == 'z') {
        ...
        for (unsigned I = 1, e = AugmentationString.size(); I != e; ++I) {
          ...
          switch (Char) {
          case 'e':
            grin_assert((I + 1) != e && AugmentationString[I + 1] == 'h',
          ...
          case 'L':
            ...
            grin_assert(!LSDAPointerEncoding, "Duplicate LSDA encoding");
            LSDAPointerEncoding = EHFrameReader.readNextU8();
            break;
          case 'P': {
            grin_assert(!PersonalityEncoding, "Duplicate personality");
            ...
            grin_log(EhFrameLog, "Personality function: " << PersonalityPtr);
            // TODO: technically this is not a landing pad
            LandingPads.insert(PersonalityPtr);
            break;
          }
          case 'R':
            grin_assert(!FDEPointerEncoding, "Duplicate FDE encoding");
            FDEPointerEncoding = EHFrameReader.readNextU8();
            break;
          case 'z':
            grin_unreachable("'z' must be first in the augmentation string");
          }
        }
      }
      ...
      grin_assert(CIEIt != CachedCIEs.end(),
      ...
      grin_assert(CIE.FDEPointerEncoding,
      ...
      grin_log(EhFrameLog, "PCBegin: " << std::hex << PCBegin);
...
void BinaryFile::parseLSDA(uint64_t FDEStart, uint64_t LSDAAddress) {
  grin_log(EhFrameLog, "LSDAAddress: " << std::hex << LSDAAddress);

  auto R = getAddressData(LSDAAddress);
  grin_assert(R, "LSDA not available in any segment");
  ...
  grin_log(EhFrameLog, "LandingPadBase: " << std::hex << LandingPadBase);
