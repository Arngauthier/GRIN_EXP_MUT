  ...
  for (const Function &F : M) {
    for (const Use &U : F.uses()) {
      ...
      if (const auto *TheCall = dyn_cast<const CallInst>(TheUser)) {
        ...
        grin_assert(CExpr->getOpcode() == Instruction::BitCast);
          ...
          for (const ConstantExpr *BitCast : CurBitCasts) {
            for (const User *BitCastUser : BitCast->users()) {
              ...
              if (TheCall) {
                ...
                grin_assert(NewCExpr->getOpcode() == Instruction::BitCast);
...
forwardTaintAnalysis(GlobalVariable *CPUStatePtr,
  ...
  grin_assert(CPUStatePtr != nullptr);
  grin_assert(CPUStatePtr->getType()->isPointerTy());
  ...
  for (const User *U : CPUStatePtr->users()) {
    ...
    grin_assert(Load->getPointerOperand() == CPUStatePtr);
    ...
    while (not ToTaintWorkList.empty()) {
      ...
      case Instruction::Load: {
        ...
        grin_assert(OperandNo == LoadInst::getPointerOperandIndex());
      ...
      case Instruction::Store: {
        ...
        grin_assert(OperandNo == StoreInst::getPointerOperandIndex());
      ...
      case Instruction::Add: {
        ...
        grin_assert(OpCode != Instruction::GetElementPtr
      ...
      case Instruction::Call: {
        ...
        grin_assert(ReachableFunctions.count(Callee) != 0);
        ...
        grin_assert(FormalArgument != nullptr);
      ...
      case Instruction::Ret: {
        ...
        // (propagation from the callee to all call sites)
        TaintLog << "RET" << DoLog;
        grin_assert(not CallSiteInfos.empty());
      ...
      while (not ToTaintWorkList.empty() and UnexploredUse == nullptr) {
        ...
        if (Arg and PoppedHasAllExploredSources) {
          ...
          const CallSiteInfo &CSInfo = CallSiteInfos.top();
          grin_assert(CSInfo.Arg == Arg);

          unsigned ArgNo = Arg->getArgNo();
          grin_assert(CSInfo.ArgNo == ArgNo);
...
public:
  ...
  explicit WorkItem(Argument *A,
    ...
    for (const Use &FUse : F->uses()) {
      ...
      if (const auto *FCall = dyn_cast<const CallInst>(FUser)) {
        ...
        grin_assert(OpCode == Instruction::BitCast);
  ...
  explicit WorkItem(CallInst *C, bool IsLoad) :
    ...
    grin_assert(F != nullptr); // Assume no indirect calls
    if (F->getIntrinsicID() == Intrinsic::memcpy) {
      ...
      for (const BasicBlock &BB : *F) {
        ...
        if (I and isa<ReturnInst>(I) and I->getNumOperands() != 0) {
          grin_assert(I->getNumOperands() == 1);
...
public:
  ...
  void fold(const WorkItem &Item, ValueCallSiteOffsetMap &OffsetMap) {
    ...
    for (const Use *U : Item.sources()) {
      ...
      grin_assert(not CallSiteOffsets.empty());
    ...
    grin_assert(NumSrcs == SrcCallSiteOffsetsPtrs.size());
    grin_assert(NumSrcs == NonRootOffsetsPtrs.size());
    ...
    for (const auto &CSOffsets : SrcCallSiteOffsetsPtrs) {
      ...
      grin_assert(found_nullptr or found_all_calls);
    }
    ...
    for (CallInst *C : CallSites) {
      ...
      grin_assert(NumSrcs < (8ULL * sizeof(uint64_t)));
      ...
      for (uint64_t i = 0; i < combinations; ++i) {
        ...
        grin_assert(not UpdatedOffsetTuple[0].hasValue());
        std::tie(Valid,
                 ResKind) = T::checkOffsetTupleIsValid(OffsetTuple,
                                                       I,
                                                       UpdatedOffsetTuple);
        grin_assert(UpdatedOffsetTuple.size() == OffsetTuple.size());
        if (not Valid) {
        ...
        for (WorkItem::size_type SI = 0; SI < NumSrcs; ++SI) {
          ...
          grin_assert(OffsetSize);
          grin_assert(CartesianSize <= CartesianSize * OffsetSize);
...
private:
  ...
  checkOffsetTupleIsValid(const SmallVector<const CSVOffsets *, 4> &OffsetTuple,
    ...
    grin_assert(OffsetTuple.size() == 2);
    auto OpCode = I->getOpcode();
    grin_assert(OpCode == Instruction::Add or OpCode == Instruction::Sub);
    const auto O0 = OffsetTuple[0], O1 = OffsetTuple[1];
    if (OpCode == Instruction::Add) {
      // Cannot add pointers
      grin_assert(not(O0->isPtr() and O1->isPtr()));
    } else {
      // Cannot subtract a pointer from something else
      grin_assert(not O1->isPtr());
    }
...
private:
  ...
  checkOffsetTupleIsValid(const SmallVector<const CSVOffsets *, 4> &OffsetTuple,
    ...
    if (GEP->hasIndices() and PointeeTy->isArrayTy()) {
      ...
      grin_assert(not(FirstIdxConst != nullptr) or FirstIdxPropagatedConst);

      if (FirstIdxPropagatedConst) {

        bool FirstIdxZero = FirstIdxConst->isZero();
        bool FirstIdxPropagatedZero = *OffsetTuple[1]->begin() == 0;
        grin_assert(not FirstIdxZero or FirstIdxPropagatedZero);
        ...
        if (FirstIdxPropagatedZero) {
          ...
          for (; IdxIt != IdxEnd; ++IdxIt, ++IdxOpNum) {
            ...
            grin_assert(not IdxCSVOffset->isPtr());
              ...
              if (ElementTy->isArrayTy()) {
                ...
                if (IdxCSVOffset->isUnknown()) {
                  if (IdxIt + 1 != IdxEnd) {
                    ...
                    grin_assert(not LastTypeOffsets.empty());
                    CSVOffsets U(CSVOffsets::Kind::Numeric, LastTypeOffsets);
                    UpdatedOffsetTuple[IdxOpNum] = std::move(U);
                    break;
                  }
                  ...
                  // If it's not Unknown we can leave it like it is.
                  grin_assert(not IdxCSVOffset->empty());
                  ConstIdxList.push_back(*IdxCSVOffset->begin());

                  grin_assert(IdxCSVOffset->size() != 0);
                  LastTypeOffsets.clear();
    ...
    for (size_t O = 1; O < NOperands; O++) {
      grin_assert(not OffsetTuple[O]->isPtr());
...
private:
  ...
  checkOffsetTupleIsValid(const SmallVector<const CSVOffsets *, 4> &OffsetTuple,
    ...
    auto OpCode = I->getOpcode();
    grin_assert(OpCode == Instruction::Shl or OpCode == Instruction::AShr
    ...
    const auto O0 = OffsetTuple[0], O1 = OffsetTuple[1];
    grin_assert(not O0->isPtr() and not O1->isPtr());
    if (O0->isUnknown() or O1->isUnknown()) {
...
void CPUSAOA::computeOffsetsFromSources(const WorkItem &Item, bool IsLoad) {
  ...
  if (isa<PHINode>(ItemVal) or isa<Argument>(ItemVal)
    ...
    if (Callee != nullptr and Callee->getIntrinsicID() == Intrinsic::memcpy) {
      ...
      grin_assert(isa<ConstantInt>(Call->getArgOperand(2)));
    ...
    default:
      grin_abort(dumpToString(Instr).data());
    }
  } else {
    grin_abort();
  }
...
void CPUSAOA::computeAggregatedOffsets() {
  ...
  for (std::pair<Value *const, CallSiteOffsetMap> &ACSO : AccessCSOffsets) {
    ...
    grin_assert(isInstr and (isCorrectAccessType or isCallToBuiltinMemcpy));
    auto *Instr = dyn_cast<Instruction>(I);
    grin_assert(Tainted.count(Instr) != 0);
...
CPUStateAccessAnalysis::setupOutEnvAccess(Instruction *AccessToFix) {
  ...
  grin_assert(InstrIt != AccessToFixBB->end());
  grin_assert(std::next(InstrIt) != AccessToFixBB->end());
...
void CPUStateAccessAnalysis::correctCPUStateAccesses() {
  ...
  for (std::pair<Instruction *const, CSVOffsets> &IOff : CSVAccessOffsetMap) {
    ...
    FixAccessLog << "Fixing access: " << I
                 ...
                 << DoLog;
    grin_assert(CSVOffsets::isPtr(OKind));
    ...
    case CSVOffsets::Kind::KnownInPtr: {
      ...
      grin_log(FixAccessLog, "Before: " << dumpToString(F));
      ...
      grin_assert(AccessToFix != nullptr);
      LLVMContext &Context = M.getContext();
      ...
      // Create the default BB for the switch, calling grin_abort()
      ...
      if (OffsetValue == nullptr)
        OffsetValue = buildEnvOffsetValue(Builder,
                                          ...
                                          Address,
                                          Int64Ty);
      grin_assert(OffsetValue != nullptr);
      ...
      if (CSVOffsets::isUnknownInPtr(OKind)) {
        ...
        grin_assert(SwitchOffset->getNumCases() > 0);
        break; // out from the switch, to the verify and cleanup code
      }

      grin_assert(Offsets.size() > 0);
      SwitchInst *SwitchOffset = Builder.CreateSwitch(OffsetValue,
                                                      Default,
                                                      Offsets.size());
    ...
    grin_log(FixAccessLog, "After: " << dumpToString(F));
    if (I != AccessToFix) {
      if (FixAccessLog.isEnabled()) {
...
bool CPUStateAccessAnalysis::run() {
  ...
  QuickMetadata QMD(Context);
  if (Found) {
    auto LoadMDKind = Context.getMDKindID("grin.csvaccess.offsets."
                                          "load");
    addAccessMetadata(CallSiteLoadOffset, Variables, QMD, LoadMDKind);
    auto StoreMDKind = Context.getMDKindID("grin.csvaccess.offsets."
                                           "store");
  ...
  grin_assert(not verifyModule(M, &dbgs()));
