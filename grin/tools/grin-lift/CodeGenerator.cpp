...
#include "grin/Support/DebugHelper.h"
...
cl::opt<bool> DebugChain("debug-chain",
                         cl::desc("Debug generate chains"),
                         cl::cat(MainCategory));
...
static void IRtoIR(PTCInstructionListPtr &InstructionList,
...
static void replaceFunctionWithRet(Function *ToReplace, uint64_t Result) {
  ...
  if (ToReplace->getReturnType()->isVoidTy()) {
    ...
    grin_unreachable("No-op functions can only return void or an integer "
                     "type");
  }
...
auto find_unique(Range &&TheRange, UnaryPredicate Predicate)
  ...
  auto Result = It;
  grin_assert(Result != End);
  grin_assert(std::find_if(++It, End, Predicate) == End);
...
auto find_unique(Range &&TheRange) -> decltype(*TheRange.begin()) {
  ...
  auto Result = Begin;
  grin_assert(Begin != End && ++Result == End);
...
bool CpuLoopFunctionPass::runOnModule(Module &M) {
  ...
  grin_assert(F.getReturnType()->isVoidTy());
  ...
  grin_assert(Footer->end() != Footer->begin());
  Instruction *LastInstruction = &*--Footer->end();
  grin_assert(isa<BranchInst>(LastInstruction));
  ...
  grin_assert(Call->getCalledFunction() == &CpuExec);
...
static ReturnInst *createRet(Instruction *Position) {
  ...
  if (ReturnType->isVoidTy()) {
    ...
    grin_assert("Return type not supported");
  }
...
bool CpuLoopExitPass::runOnModule(llvm::Module &M) {
  ...
  while (!CpuLoopExitUsers.empty()) {
    ...
    grin_assert(Call->getCalledFunction() == CpuLoopExit);
      ...
      while (!WorkList.empty()) {
        ...
        for (User *RecUser : F->users()) {
          ...
          if (RecCall == nullptr) {
            ...
            grin_assert(Cast != nullptr, "Unexpected user");
            grin_assert(Cast->getOperand(0) == F && Cast->isCast());
            WorkList.push(Cast);
            continue;
          }
          ...
          if (RecCaller->getName() == "root") {
            ...
            grin_assert(SplitPoint != OldBB->end());
...
void CodeGenerator::translate(uint64_t VirtualAddress) {
  ...
  //
  // Create grin.input named metadata
  //

  const char *MDName = "grin.input.canonical-values";
  ...
  // Currently grin.input.architecture is composed as follows:
  //
  // grin.input.architecture = {
  ...
  MDName = "grin.input.architecture";
  ...
  JumpTargets.textStartAddr = Binary.textStartAddr;
  JumpTargets.textEndAddr = Binary.textEndAddr;
  ...
  if (DebugChain) {
    ...
    /* TODO:
     *  JumpTargets.testFunction();
     */
    JumpTargets.readDebugInfo(OutputPath);
    JumpTargets.testDebugFunction();
    exit(0);
  }
  ...
  while (Entry != nullptr) {
    ...
    if (*ptc.exception_syscall == 0x100) {
      ...
      DynamicVirtualAddress = traverseFLAG ? *ptc.syscall_next_eip :
                                             ptc.do_syscall2();
    ...
    if (*ptc.exception_syscall == 11) {
      if (!JumpTargets.haveBB) {
        DynamicVirtualAddress = JumpTargets
                                  .handleIllegalMemoryAccess(BlockBRs,
                                                             tmpVA,
                                                             ConsumedSize);
      } else
    ...
    JumpTargets.generatePartCFG(tmpVA, DynamicVirtualAddress, BlockBRs);
    JumpTargets.generatePartCFGWithNext(tmpVA, DynamicVirtualAddress, BlockBRs);
    ...
    if (!traverseFLAG) {
      ...
      if (DynamicVirtualAddress) {
        auto tmpBB = JumpTargets.registerJT(DynamicVirtualAddress,
                                            JTReason::GlobalData);
        if (JumpTargets.haveBB) {
      ...
      } else if (DynamicVirtualAddress == 0
                 && !JumpTargets.BranchTargets.empty()) {
        // Enable branch traversal
        traverseFLAG = 1;
        selectNewBranchEntry(JumpTargets,
                             DynamicVirtualAddress,
                             srcBB,
                             srcAddr);
      }
  ...
  for (auto Name : AbortFunctionNames) {
    ...
    if (TheFunction != nullptr) {
      grin_assert(HelpersModule->getFunction("abort") != nullptr);
  ...
  if (not NoLink) {
    ...
    grin_assert(!Result, "Linking failed");
  }
  ...
  if (Function *InitEnv = TheModule->getFunction("initialize_env")) {
    grin_assert(not InitEnv->getFunctionType()->isVarArg());
    grin_assert(InitEnv->getFunctionType()->getNumParams() == 1);
...
static void selectNewBranchEntry(JumpTargetManager &JumpTargets,
  ...
  std::tie(DynamicVirtualAddress,
           srcBB,
           srcAddr) = JumpTargets.BranchTargets.front();
