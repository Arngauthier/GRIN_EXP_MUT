...
#include "grin/Support/MemoryAccess.h"
#include "grin/Support/Transform.h"
...
BoundedValue OSRA::mergePredicate(OSR &BaseOp, Predicate P, Constant *ConstOp) {
  ...
  default:
    grin_unreachable("Unexpected comparison operator");
  }
}
...
void OSRA::handleComparison(Instruction *I) {
  ...
  if (P == CmpInst::ICMP_EQ || P == CmpInst::ICMP_NE) {
    grin_assert(Constraints.count(nullptr) == 0);
  ...
  for (auto &LHSPair : LHS.Constants) {
    for (auto &RHSPair : RHS.Constants) {
      ...
      if (LHSPair.second != nullptr) {
        ...
        grin_assert(RHSPair.second != nullptr);
...
void OSRA::handleBranch(Instruction *I) {
    ...
    for (const User *U : AffectedInst->users()) {
      if (auto *I = dyn_cast<const Instruction>(U)) {
        ...
        case Instruction::Load:
          ...
          break;
        default:
          grin_assert(isa<TerminatorInst>(I), "Unexpected instruction");
          AffectedSet.insert(I->getParent());
  ...
  while (!ConstraintsWL.empty()) {
    ...
    grin_assert(BlockBlackList.find(Entry.Target) == BlockBlackList.end());
...
void OSRAPass::releaseMemory() {
  grin_log(ReleaseLog, "OSRAPass is releasing memory");
  freeContainer(OSRs);
  if (BVs) {
    delete BVs;
    BVs = nullptr;
...
OSRAPass::identifyOperands(std::map<const Value *, const OSR> &OSRs,
                           const Instruction *I,
                           const DataLayout &DL) {
  grin_assert(I->getNumOperands() == 2);
  ...
  if (ReachersIt != LoadReachers.end()) {
    ...
    if (ReacherIt != Reachers.end()) {
      ...
      if (ReacherIt->second == NewOSR
        ...
        grin_assert(!(Reachers.size() > 1 && ReacherValue == Load
...
BoundedValue OSRA::pathSensitiveMerge(LoadInst *Reached) {
  ...
  grin_log(Log, "Performing PSM for " << getName(Reached));
  ...
  while (!Stack.empty()) {
    ...
    grin_log(Log, Indent << "Exploring " << getName(Pred));
    ...
    for (Reacher &R : Reachers) {
      ...
      if (R.isLTR(Pred)) {
        ...
        grin_log(Log, Indent << "  Deactivating reacher " << ReacherIndex);
    ...
    if (not(NewPredIt != filtered_pred_end(Pred)))
      grin_log(Log, Indent << "    No predecessors");
  ...
  grin_log(Log, "FinalBV: " << FinalBV.describe());

  grin_assert(not FinalBV.isUninitialized());
  return FinalBV;
}
...
bool OSRAPass::runOnModule(Module &M) {
  grin_log(PassesLog, "Starting OSRAPass");
  ...
  grin_log(PassesLog, "Ending OSRAPass");
  return false;
}
...
BoundedValue &BVMap::summarize(BasicBlock *Target, MapValue *BVOVector) {
  ...
  grin_assert(PredecessorsCount >= BVOVector->Components.size());
...
public:
  ...
  getBV(const BoundedValue &Base, boost::icl::interval_set<T> Intervals) {
    ...
    if (Intervals.iterative_size() == 0) {
      ...
      grin_assert(Result.Bounds.size() == 0 || Result.Bounds.size() == 1);
      Result.Bounds.clear();
      for (auto Interval : Intervals) {
        const auto static_closed = interval_bounds::static_closed;
        grin_assert(Interval.bounds().bits() == static_closed);
        ...
        for (auto &Pair : Result.Bounds) {
          if (&Pair != &Result.Bounds.back()) {
            grin_assert(Pair != Result.Bounds.back());
          }
        }
      }
      grin_assert(Result.Bounds.size() != 0);
    }
...
bool BoundedValue::slowCompare(const BoundedValue &Other) const {
  grin_assert(hasSignedness() && Other.hasSignedness());
  grin_assert(Sign == Other.Sign);
