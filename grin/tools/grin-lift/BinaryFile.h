...
inline SymbolType::Values fromELF(unsigned char ELFSymbolType) {
  ...
  default:
    grin_abort("Unexpected symbol type");
  }
}
...
public:
  ...
  uint64_t value() const {
    grin_assert(isAbsoluteValue() or isBaseRelativeValue());
    return Value;
  }

  llvm::StringRef symbolName() const {
    grin_assert(isSymbolRelativeValue() or isSymbol());
    return SymbolName;
  }

  uint64_t offset() const {
    grin_assert(isSymbolRelativeValue());
    return Value;
  }
...
public:
  ...
  uint64_t getPointer(Pointer Ptr) const {
    ...
    grin_assert(R, "Pointer not available in any segment");
