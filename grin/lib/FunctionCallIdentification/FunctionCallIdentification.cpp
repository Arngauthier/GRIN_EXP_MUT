...
#include "grin/FunctionCallIdentification/FunctionCallIdentification.h"
...
bool FunctionCallIdentification::runOnModule(llvm::Module &M) {
  grin_log(PassesLog, "Starting FunctionCallIdentification");
  ...
  if (FunctionCall->empty()) {
    ...
    grin_assert(FunctionCall->user_begin() == FunctionCall->user_end());
  }
          ...
          if (auto *Store = dyn_cast<StoreInst>(&I)) {
            ...
            if (GCBI.isPCReg(TargetCSV)) {
              ...
              if (Constant->getLimitedValue() == ReturnPC) {
                ...
                grin_assert(LinkRegister == nullptr);
                if (TargetCSV != nullptr) {
                  ...
                  grin_assert(LastStackPointer != nullptr);
                  grin_assert(skipCasts(LastStackPointer) == Pointer);
  ...
  grin_log(PassesLog, "Ending FunctionCallIdentification");
