...
bool GeneratedCodeBasicInfo::runOnModule(llvm::Module &M) {
  ...
  grin_log(PassesLog, "Starting GeneratedCodeBasicInfo");

  RootFunction = &F;

  const char *MDName = "grin.input.architecture";
  ...
  for (BasicBlock &BB : F) {
    if (!BB.empty()) {
      ...
      case DispatcherBlock:
        grin_assert(Dispatcher == nullptr);
        Dispatcher = &BB;
        break;

      case DispatcherFailureBlock:
        grin_assert(DispatcherFail == nullptr);
        DispatcherFail = &BB;
        break;

      case AnyPCBlock:
        grin_assert(AnyPC == nullptr);
        AnyPC = &BB;
        break;
  ...
  grin_assert(Dispatcher != nullptr && AnyPC != nullptr
               && UnexpectedPC != nullptr);

  grin_log(PassesLog, "Ending GeneratedCodeBasicInfo");
