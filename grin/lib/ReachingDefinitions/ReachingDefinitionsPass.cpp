...
#include "grin/ReachingDefinitions/ReachingDefinitionsAnalysisImpl.h"
...
bool ReachingDefinitionsPass::runOnModule(llvm::Module &M) {
  grin_log(PassesLog, "Starting ReachingDefinitionsPass");
  ...
  grin_log(PassesLog, "Ending ReachingDefinitionsPass");
...
bool ConditionalReachedLoadsPass::runOnModule(llvm::Module &M) {
  grin_log(PassesLog, "Starting ConditionalReachedLoadsPass");
  ...
  grin_log(PassesLog, "Ending ConditionalReachedLoadsPass");
...
bool ConditionNumberingPass::runOnModule(Module &M) {
  ...
  grin_log(PassesLog, "Starting ConditionNumberingPass");
  ...
  for (auto &P : Conditions) {
    ...
    for (BranchInst *T : Sisters) {
      ...
      for (BasicBlock *Successor : Successors) {
        grin_assert(Successor->getSinglePredecessor() == T->getParent());
  ...
  for (BasicBlock *BB : ToDelete) {
    auto It = BB->begin();
    auto End = BB->end();
    grin_assert(It != End and isa<BranchInst>(&*It));
    It++;
    grin_assert(It == End);

    BasicBlock *Successor = BB->getSingleSuccessor();
    BasicBlock *Predecessor = BB->getSinglePredecessor();
    grin_assert(Successor != nullptr and Predecessor != nullptr);

    BB->replaceAllUsesWith(Successor);
    BB->eraseFromParent();
  }

  grin_log(PassesLog, "Ending ConditionNumberingPass");
