...
#include "grin/FunctionIsolation/IsolateFunctions.h"
#include "grin/Runtime/commonconstants.h"
#include "grin/StackAnalysis/FunctionsSummary.h"
...
void IFI::throwException(Reason Code, BasicBlock *BB, uint64_t AdditionalPC) {
  grin_assert(PC != nullptr);
  grin_assert(RaiseException != nullptr);
  grin_assert(DebugException != nullptr);
...
bool IFI::replaceFunctionCall(BasicBlock *NewBB,
  ...
  if (IsIndirect) {
    ...
    if (Node == nullptr) {
      ...
      grin_assert(not isTerminatorWithInvalidTarget(T, RootToIsolated));
  ...
  grin_assert(TargetFunction != nullptr);
    ...
    while (!UserQueue.empty()) {
      ...
      for (Use &CurrentUse : CurrentUser->operands()) {
        ...
        if (ReplacementIt != RootToIsolated.end()) {
          ...
          grin_assert(RootToIsolated.count(CurrentOperand) != 0);
...
void IFI::run() {
  ...
  grin_assert(CallMarker != nullptr);
  ...
  for (BasicBlock &BB : *RootFunction) {
    ...
    if (MDNode *Node = Terminator->getMetadata("func.entry")) {
      ...
      grin_assert(Functions.count(FunctionNameMD) == 0);
  ...
  for (BasicBlock &BB : *RootFunction) {
    ...
    if (MDNode *Node = Terminator->getMetadata("func.member.of")) {
      ...
      for (const MDOperand &Op : Tuple->operands()) {
        ...
        grin_assert(ParentFunction != nullptr);
  ...
  for (auto &Pair : Functions) {
    ...
    for (BasicBlock &NewBB : *AnalyzedFunction) {
      ...
      for (BasicBlock *Successor : Terminator->successors()) {
        ...
        grin_assert(GCBI.isTranslated(Successor)
        ...
        // We add a successor if it is not a grin block type and it is present
      ...
      grin_assert(NewBB.getTerminator() == nullptr);
    ...
    for (auto &P : Descriptor.Members) {
      ...
      grin_assert(isa<UnreachableInst>(T));
      T->eraseFromParent();
    ...
    grin_assert(not AnalyzedFunction->isDeclaration());
    ...
    for (BasicBlock &BB : *AnalyzedFunction) {
      ...
      if (&BB != UnexpectedPC && &BB != AnyPC) {
        ...
        grin_assert(isa<SwitchInst>(I) || isa<UnreachableInst>(I));
        I.eraseFromParent();
      }
    }
    ...
    for (Instruction *OldAlloca : AllocasToClone) {
      ...
      grin_assert(NewAlloca->getParent() == Dummy);
  ...
  for (BasicBlock &BB : *Root) {
    if (BB.getName() == "unexpectedpc")
      grin_assert(GCBI.getType(&BB) == UnexpectedPCBlock);
    if (GCBI.getType(&BB) == UnexpectedPCBlock) {
      UnexpectedPC = &BB;
      break;
    }
  }
  grin_assert(UnexpectedPC != nullptr);
  ...
  // by grin a call to the newly created corresponding LLVM function
  for (BasicBlock &BB : *Root) {
    grin_assert(!BB.empty());
  ...
  grin_assert(verifyModule(*TheModule, &Stream) == false);
}
