...
#include "grin/Support/Statistics.h"
...
void InterproceduralAnalysis::run(BasicBlock *Entry, ResultsPool &Results) {
  ...
  if (Cached) {
    ...
    if (Type == FunctionType::Regular)
      grin_assert(Summary.BranchesType.size() != 0);
  ...
  grin_log(SaInterpLog, "Running interprocedural analysis on " << Entry);
  ...
  do {
    ...
    grin_assert(Result.requiresInterproceduralHandling());

    switch (Result.type()) {
    case BranchType::FakeFunction:
      grin_log(SaInterpLog, Current.entry() << " is fake");
    ...
    case BranchType::UnhandledCall: {
      ...
      if (getRecursionRoot(Callee) != nullptr) {
        ...
        grin_assert(TheCache.get(Callee));
    ...
    case BranchType::FunctionSummary: {
      ...
      grin_log(SaInterpLog, "We have a summary for " << Current.entry());
      ...
      if (Offending.size() != 0) {
        ...
        for (BasicBlock *Entry : Offending) {
          ...
          grin_assert(Current.entry() != Entry);
        ...
        grin_assert(TheCache.get(Current.entry()));
      }

      if (SaLog.isEnabled()) {
        grin_log(SaLog, "FinalState for " << getName(Current.entry()));
        Summary.dump(getModule(Entry), SaLog);
      }

      if (MustReanalyze) {
        grin_log(SaInterpLog, "Something has changed, let's reanalyze");
        ...
        case BranchType::NoReturnFunction:
          grin_log(SaInterpLog, Current.entry() << " doesn't return");
    ...
    case BranchType::Invalid:
      grin_abort("Unexpected branch type in interprocedural analysis");
    }

  } while (InProgress.size() > 0);

  grin_assert(Type != FunctionType::Invalid);
...
void ResultsPool::mergeFunction(BasicBlock *Function,
  ...
  for (auto &Slot : Slots) {
    ...
    grin_assert(Slot.first.addressSpace() == ASID::cpuID()
  ...
  for (auto &P : ToImport) {
    ...
    if (It != CallSites.end()) {
      if (!compareOptional(It->second, P.second)) {
        grin_abort("This call site has a stack at a different height than"
                    " previously recorded");
      }
    } else {
  ...
  static ClobberedMap run(ResultsPool &This) {
    ...
    do {
      ...
      for (int32_t Clobbered : LastResult.Clobbered[nullptr])
        grin_assert(IndirectCallClobbered.count(Clobbered) != 0);
...
FunctionsSummary ResultsPool::finalize(Module *M) {
  ...
  for (auto &P : FunctionCallSlots) {
    CallSite Call = P.first;
    grin_assert(CallSites.count(Call) != 0);
    Instruction *I = Call.callInstruction();
    ...
    for (ASSlot Slot : P.second) {
      ...
      grin_assert(Slot.addressSpace() == CPU);
