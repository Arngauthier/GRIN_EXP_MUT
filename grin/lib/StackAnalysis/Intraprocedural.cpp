...
void Analysis::initialize() {
  ...
  grin_log(SaLog, "Creating Analysis for " << getName(Entry));
...
public:
  ...
  void set(Instruction *I, Value V) {
    grin_assert(I != nullptr);
    grin_assert(I->getParent() == BB,
                 "Instruction from an unexpected basic block");
    grin_assert(InstructionContent.count(I) == 0,
...
Interrupt Analysis::transfer(BasicBlock *BB) {
  ...
  grin_assert(It != State.end());
  Element Result = It->second.copy();

  grin_log(SaBBLog, "Analyzing " << getName(BB));
  ...
  for (Instruction &I : *BB) {
    ...
    grin_log(SaVerboseLog, "NewInstruction: " << getName(&I));
    ...
    case Instruction::Call: {
      ...
      grin_assert(isCallToHelper(&I) || isMarker(&I)
...
Interrupt Analysis::handleTerminator(TerminatorInst *T,
  ...
  grin_assert(not isa<UnreachableInst>(T));
...
Interrupt Analysis::handleCall(Instruction *Caller,
  ...
  grin_assert(not(IsRecursive && IsIndirect));
  ...
  grin_assert(not(IsIndirectTailCall and IsKiller));
  if (IsIndirectTailCall) {
    return AI::create(std::move(Result), BT::IndirectTailCall);
  } else if (IsKiller) {
    return AI::create(std::move(Result), BT::Killer);
  } else {
    grin_assert(ReturnFromCall != nullptr);
...
IFS Analysis::createSummary() {
  ...
  if (AnalyzeABI) {
    ...
    if (SaABI.isEnabled()) {
      grin_log(SaABI, "Starting analysis of " << Entry);
      TheABIIR.dump(SaABI, M);
      SaABI << DoLog;
    }

    grin_assert(TheABIIR.verify(), "The ABI IR is invalid");
