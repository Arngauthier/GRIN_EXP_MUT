...
public:
  ...
  FunctionCall call() const {
    grin_assert(isCall());
    grin_assert(Call.callInstruction() != nullptr);
    return Call;
  }
...
public:
  ...
  void clear() {
    grin_assert(Predecessors.empty());
    Instructions.clear();
    Successors.clear();
    IsReturn = false;
  }
