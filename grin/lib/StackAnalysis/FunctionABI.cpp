...
public:
  ...
  unsigned cmp(const Element &Other, const Module *M = nullptr) const {
    ...
    ROA((registerCmp(RegisterAnalyses, Other.RegisterAnalyses, CPU, M)),
        { grin_log(SaDiffLog, "RegisterAnalyses"); });
...
public:
  ...
  bool isReturn() const {
    grin_assert(TheReason == Regular or TheReason == Return);
...
public:
  ...
  void initialize() {
    grin_assert(FirstRun, "The ABIAnalysis cannot be run twice");
    FirstRun = false;
    Base::initialize();
  }
  ...
  Interrupt<E> transfer(ABIIRBasicBlock *BB) {
    grin_log(SaABI, "Analyzing " << BB->basicBlock());
...
void FunctionABI::analyze(const ABIFunction &TheFunction) {
  ...
  {
    grin_log(SaABI, "Running forward function analyses");
