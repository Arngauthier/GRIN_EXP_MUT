...
public:
  ...
  static Interrupt createWithSuccessor(Element Result,
    ...
    grin_assert(Type == BranchType::FakeFunctionCall
  ...
  static Interrupt createWithSuccessors(Element Result,
                                        BranchType::Values Type,
                                        vector Successors) {
    grin_assert(Type == BranchType::InstructionLocalCFG
                 || Type == BranchType::FunctionLocalCFG);
    grin_assert(Successors.size() > 0);
  ...
  static Interrupt create(Element Result, BranchType::Values Type) {
    ...
    grin_assert(Type == Return || Type == IndirectTailCall
...
public:
  ...
  Element &&extractResult() {
    grin_assert(Type != BranchType::FunctionSummary
                 and Type != BranchType::UnhandledCall);

    grin_assert(not ResultExtracted);
    ResultExtracted = true;
    return std::move(Result);
  }

  llvm::BasicBlock *getCallee() const {
    grin_assert(Type == BranchType::UnhandledCall);
    grin_assert(RelatedBasicBlocks.size() == 1);
...
public:
  ...
  llvm::Optional<int32_t> frameSizeAt(FunctionCall Location) const {
    ...
    grin_assert(It != FrameSizeAtCallSite.end(),
