...
public:
  ...
  void insert(Iterated Entry) {
    ...
    grin_assert(It != PostOrderListIndex.end());
  ...
  Iterated head() const {
    grin_assert(Next != InvalidIndex);
    return PostOrderList[Next].entry();
  }

  Iterated pop() {
    grin_assert(not empty());
    grin_assert(verify());
...
public:
  ...
  Interrupt run() {
      ...
      if (Result.isReturn()) {
        ...
        if (DynamicGraph) {
          ...
          if (Successors->size() != 0)
            grin_assert(NewSuccessors == *Successors);
    ...
    if (DynamicGraph)
      grin_assert(FirstFinalResult == (FinalStates.size() == 0));
    else
      grin_assert(FinalStates.size() == 0 and SuccessorsMap.size() == 0);

    // The work list is empty
    grin_assert(ToVisit.empty());
    if (FirstFinalResult) {
      ...
      if (DynamicGraph) {
        ...
        while (not ReachableLabels.empty()) {
          ...
          grin_assert(It != SuccessorsMap.end());
        ...
        grin_assert(Reachable.size() == State.size());
