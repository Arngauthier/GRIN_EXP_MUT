...
private:
  ...
  unsigned dereference() const {
    grin_assert(BitVector != nullptr && NextBitIndex != 0);
...
private:
  ...
  struct LargeStorage {
    ...
    void zero(size_t From, size_t Count) {
      grin_assert(From + Count <= wordCount());
    ...
    LargeStorage &operator=(const LargeStorage &Other) {
      grin_assert(Capacity >= Other.Capacity);
...
public:
  ...
  void set(unsigned Index) {
    grin_assert(Index < IntMax);

    if (Index >= capacity())
      alloc(Index + 1);

    if (isSmall()) {
      grin_assert(Index < MaxSmallSize);
  ...
  void zero(size_t From, size_t Count) {
    ...
    if (isSmall()) {
      ...
      if (HeadComplement != 0) {
        ...
        From += Head;
        Count -= Head;
        grin_assert(From % BitsPerPointer == 0);
      }
      ...
      if (Tail != 0) {
        ...
        Count -= Tail;
        grin_assert(Count % BitsPerPointer == 0);
      }
  ...
  LazySmallBitVector &operator^=(const LazySmallBitVector &Other) {
    ...
    grin_assert(!(isSmall() && !Other.isSmall()));
...
inline void LazySmallBitVectorIterator<LSBV>::increment() {
  grin_assert(BitVector != nullptr);
  grin_assert(NextBitIndex == 0 || (*BitVector)[NextBitIndex - 1] == true);
