...
time -p grin-lift $GOBMK.stripped $GOBMK.ll -multi-process-chains -process-nums=$process -loop-nums=1154 -exe-args="-v" 2>$GOBMK.log 1>/dev/null
rm $GOBMK.stripped
echo "403"
cp $GCC $GCC.stripped
strip $GCC.stripped
...
strip $H264.stripped
time -p grin-lift $H264.stripped $H264.ll -multi-process-chains -process-nums=$process -loop-nums=5 -exe-args="-h" 2>$H264.log 1>/dev/null
rm $H264.stripped
